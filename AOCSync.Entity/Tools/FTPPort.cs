...
namespace AOCSync.Entity.Tools
{
    ...
    public class FTPClientPort
    {
        ...
        #region 构造函数
        public FTPClientPort(String server, String port, String filePath, String user, String pass)
        {
...
//            {
                ...
                try
                {
                    ...
                    if (timeout > 0)
                    {
                        try
                        {
                            listening_sock.SetSocketOption(SocketOptionLevel.Socket,
                        ...
                        {
                           errormessage +="Failed to set socket timeout: " + ex.Message;
                        }
                    }
                    //设置超时
        ...
        private void ConnectDataSocket()
        {
            ...
            try
            {
                bool success = listening_sock.Poll(timeout, SelectMode.SelectRead);
                if (!success)
                {
                ...
                if (timeout > 0)
                {
                    try
                    {
                        data_sock.SetSocketOption(SocketOptionLevel.Socket,
                            SocketOptionName.ReceiveTimeout, timeout);
                        data_sock.SetSocketOption(SocketOptionLevel.Socket,
                            SocketOptionName.SendTimeout, timeout);
                    }
...
//#endif
        ...
        public bool Upload(AOCUserData aocUserData)
        {
            ...
            {
                strRemotePath = aocUserData.FTPRemoteDir;
            }
