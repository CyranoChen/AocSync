...
{
    public class AOCDataFQS : IComparable
    {
        public AOCDataFQS() { }

        public AOCDataFQS(DataRow dr)
        {
            initAoc(dr);
        }
        ...
        public void Select()
        {
            DataRow dr = DataAccess.AOCDataFQS.GetAOCDataByID(ID);
        ...
        public void Update()
        {
            DataAccess.AOCDataFQS.UpdateAOCData(
                ...
                GATE_PLAN_CLOSE_TIME1, GATE_PLAN_OPEN_TIME1, GATE_PLAN_CLOSE_TIME2, GATE_PLAN_OPEN_TIME2, USEC, TYPC, CDAT, USEU, TYPU, LSTU, JFNO, CTRL, DRCT);
        }
        ...
        public void Insert()
        {
            DataAccess.AOCDataFQS.InsertAOCData(
                URNO, CARRIER_IATA, CARRIER_ICAO, FL_NUMBER, FL_SUFFIX, FLT, FLR, SDT, SDP, LINKEDFL_FLT, LINKEDFL_CARRIER,
                LINKEDFL_NUMBER, LINKEDFL_SUFFIX, LINKEDFL_SDT, LINKEDFL_FLR, LINKFL_ID, MASTERFL_CARRIER, MASTERFL_NUMBER, MASTERFL_SUFFIX, MFDI,
                MASTERFL_FLR, MASTERFL_ID, AIRCRAFT_CALLSIGN, AIRCRAFT_OWNER, AIRCRAFT_PAXCAP, REGN, ATC5, AIRCRAFT_TYPE, AIRCRAFT_OPERATOR,
                AIRCRAFT_TERMINAL, AIRPORT_IATA, BRC1, RECLAIM_ROLE1, BROD, BRCD, EXITDOOR1, SRC2, RECLAIM_ROLE2, SBRO, SBRC, EXITDOOR2, MAKEUP_ID1, MAKEUP_ROLE1,
                MAKEUP_OPEN_TIME1, MAKEUP_CLOSE_TIME1, MAKEUP_ID2, MAKEUP_ROLE2, MAKEUP_OPEN_TIME2, MAKEUP_CLOSE_TIME2, ISBUSREQUIRED, CC_TYPE1, CKIF, CC_ROLE1,
        ...
        public void Delete()
        {
            DataAccess.AOCDataFQS.DeleteAOCData(ID);
        }

        public void Delete(SqlTransaction trans)
        {
            DataAccess.AOCDataFQS.DeleteAOCData(ID, trans);
        }

        public static void DeleteAOCDataFQSByDateTime(DateTime dateTimeStart, DateTime dateTimeEnd, SqlTransaction trans)
        {
            DataAccess.AOCDataFQS.DeleteAOCData(dateTimeStart, dateTimeEnd, trans);
        }
        public static void DeleteAOCDataFQSByDateTime( DateTime dateTimeEnd, SqlTransaction trans)
        {
            DataAccess.AOCDataFQS.DeleteAOCData( dateTimeEnd, trans);
        }

        public static List<AOCDataFQS> GetAOCDatas()
        {
            DataTable dt = DataAccess.AOCDataFQS.GetAOCDatas();
            List<AOCDataFQS> list = new List<AOCDataFQS>();

            if (dt != null)
            {
                foreach (DataRow dr in dt.Rows)
                {
                    list.Add(new AOCDataFQS(dr));
                }
            }
        ...
        public static List<PropertyInfo> GetAOCProperties()
        {
            ...
            Type typeAOC = typeof(AOCDataFQS);
        ...
        public static class Cache
        {
            ...
            public static AOCDataFQS Load(Guid id)
            {
                return AOCDataList.Find(delegate(AOCDataFQS a) { return a.ID.Equals(id); });
            }

            /// <summary>
            /// AOC数据列表
            /// </summary>
            public static List<AOCDataFQS> AOCDataList;
        ...
        //public string FL_CLASSIFICATIONCODE { set; get; }
        //public string FL_STATUSCODE { set; get; }
        ...
        public int CompareTo(object obj)
        {
            ...
            try
            {
                AOCDataFQS sObj = (AOCDataFQS)obj;
                res = this.FLT.CompareTo(sObj.FLT);//当前航班号              
                if (res == 0)
                {
                    //res = this.SDT.CompareTo(sObj.SDT);//日期
                    ...
                    if (res == 0)
                    {
                        ...
                        //{
                            ...
                            //if(res ==0)
                            //{
                            //if (string.IsNullOrEmpty(this.STOD) || string.IsNullOrEmpty(sObj.STOD))
                            //{
                            //}
                           // else
                            //{
